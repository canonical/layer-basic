...
class RelationBase(with_metaclass(AutoAccessors, object)):
    ...
    def _find_impl(cls, role, interface):
        ...
        try:
            filename = os.path.join(hooks_dir, 'relations', interface, role + '.py')
            modname = 'jujuinterfaces_%s_%s' % (interface, role)
            if modname in sys.modules:
                module = sys.modules[modname]
            else:
                module = load_source(modname, filename)
    ...
    def set_local(self, key=None, value=None, data=None, scope=None, **kwdata):
        """
        Locally store some data associated with the :class:`Conversation` with the given scope.
